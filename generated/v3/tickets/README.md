...
```go
import tickets "github.com/GIT_USER_ID/GIT_REPO_ID"
```
...
For using other server than the one defined on index 0 set context value `tickets.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), tickets.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `tickets.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), tickets.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `tickets.ContextOperationServerIndices` and `tickets.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), tickets.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), tickets.ContextOperationServerVariables, map[string]map[string]string{
...
*GDPRAPI* | [**PostCrmV3ObjectsTicketsGdprDeletePurge**](docs/GDPRAPI.md#postcrmv3objectsticketsgdprdeletepurge) | **Post** /crm/v3/objects/tickets/gdpr-delete | GDPR DELETE
...
```go
auth := context.WithValue(context.Background(), tickets.ContextAccessToken, "ACCESSTOKENSTRING")
...
auth := context.WithValue(oauth2.NoContext, tickets.ContextOAuth2, tokenSource)
r, err := client.Service.Operation(auth, args)
```
...
auth := context.WithValue(
		context.Background(),
		tickets.ContextAPIKeys,
		map[string]tickets.APIKey{
