...
For using other server than the one defined on index 0 set context value `quotes.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `quotes.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `quotes.ContextOperationServerIndices` and `quotes.ContextOperationServerVariables` context maps.
...
```go
auth := context.WithValue(context.Background(), quotes.ContextAccessToken, "ACCESSTOKENSTRING")
...
auth := context.WithValue(oauth2.NoContext, quotes.ContextOAuth2, tokenSource)
r, err := client.Service.Operation(auth, args)
```
...
auth := context.WithValue(
		context.Background(),
		quotes.ContextAPIKeys,
		map[string]quotes.APIKey{
