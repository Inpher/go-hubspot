...
For using other server than the one defined on index 0 set context value `companies.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `companies.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `companies.ContextOperationServerIndices` and `companies.ContextOperationServerVariables` context maps.
...
```go
auth := context.WithValue(context.Background(), companies.ContextAccessToken, "ACCESSTOKENSTRING")
...
auth := context.WithValue(oauth2.NoContext, companies.ContextOAuth2, tokenSource)
r, err := client.Service.Operation(auth, args)
```
...
auth := context.WithValue(
		context.Background(),
		companies.ContextAPIKeys,
		map[string]companies.APIKey{
