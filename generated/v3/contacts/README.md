...
For using other server than the one defined on index 0 set context value `contacts.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `contacts.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `contacts.ContextOperationServerIndices` and `contacts.ContextOperationServerVariables` context maps.
...
```go
auth := context.WithValue(context.Background(), contacts.ContextAccessToken, "ACCESSTOKENSTRING")
...
auth := context.WithValue(oauth2.NoContext, contacts.ContextOAuth2, tokenSource)
r, err := client.Service.Operation(auth, args)
```
...
auth := context.WithValue(
		context.Background(),
		contacts.ContextAPIKeys,
		map[string]contacts.APIKey{
