...
For using other server than the one defined on index 0 set context value `communications_status.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `communications_status.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `communications_status.ContextOperationServerIndices` and `communications_status.ContextOperationServerVariables` context maps.
...
```go
auth := context.WithValue(context.Background(), communications_status.ContextAccessToken, "ACCESSTOKENSTRING")
...
auth := context.WithValue(oauth2.NoContext, communications_status.ContextOAuth2, tokenSource)
r, err := client.Service.Operation(auth, args)
```
...
auth := context.WithValue(
		context.Background(),
		communications_status.ContextAPIKeys,
		map[string]communications_status.APIKey{
