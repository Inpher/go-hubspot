...
For using other server than the one defined on index 0 set context value `feedback_submissions.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `feedback_submissions.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `feedback_submissions.ContextOperationServerIndices` and `feedback_submissions.ContextOperationServerVariables` context maps.
...
```go
auth := context.WithValue(context.Background(), feedback_submissions.ContextAccessToken, "ACCESSTOKENSTRING")
...
auth := context.WithValue(oauth2.NoContext, feedback_submissions.ContextOAuth2, tokenSource)
r, err := client.Service.Operation(auth, args)
```
...
auth := context.WithValue(
		context.Background(),
		feedback_submissions.ContextAPIKeys,
		map[string]feedback_submissions.APIKey{
