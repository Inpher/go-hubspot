...
For using other server than the one defined on index 0 set context value `deals.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `deals.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `deals.ContextOperationServerIndices` and `deals.ContextOperationServerVariables` context maps.
...
*GDPRAPI* | [**PostCrmV3ObjectsDealsGdprDeletePurge**](docs/GDPRAPI.md#postcrmv3objectsdealsgdprdeletepurge) | **Post** /crm/v3/objects/deals/gdpr-delete | GDPR DELETE
...
```go
auth := context.WithValue(context.Background(), deals.ContextAccessToken, "ACCESSTOKENSTRING")
...
auth := context.WithValue(oauth2.NoContext, deals.ContextOAuth2, tokenSource)
r, err := client.Service.Operation(auth, args)
```
...
auth := context.WithValue(
		context.Background(),
		deals.ContextAPIKeys,
		map[string]deals.APIKey{
