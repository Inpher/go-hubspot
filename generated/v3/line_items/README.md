...
For using other server than the one defined on index 0 set context value `line_items.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `line_items.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `line_items.ContextOperationServerIndices` and `line_items.ContextOperationServerVariables` context maps.
...
```go
auth := context.WithValue(context.Background(), line_items.ContextAccessToken, "ACCESSTOKENSTRING")
...
auth := context.WithValue(oauth2.NoContext, line_items.ContextOAuth2, tokenSource)
r, err := client.Service.Operation(auth, args)
```
...
auth := context.WithValue(
		context.Background(),
		line_items.ContextAPIKeys,
		map[string]line_items.APIKey{
