...
For using other server than the one defined on index 0 set context value `transactional.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `transactional.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `transactional.ContextOperationServerIndices` and `transactional.ContextOperationServerVariables` context maps.
...
```go
auth := context.WithValue(context.Background(), transactional.ContextAccessToken, "ACCESSTOKENSTRING")
...
auth := context.WithValue(oauth2.NoContext, transactional.ContextOAuth2, tokenSource)
r, err := client.Service.Operation(auth, args)
```
...
auth := context.WithValue(
		context.Background(),
		transactional.ContextAPIKeys,
		map[string]transactional.APIKey{
